...
| Storage-v2 recovery / Session 13 (2026-09-02, corrected 15:02 CEST) | The archive datadir has run with **`storage_v2=true` continuously since at least 2026-08-14**; no manual layout transition occurred. An `Execution` consistency check nevertheless found missing receipt static-file data at **71 185 160**, forcing dependent stages from **174 027 661** to **71 185 159**. The subsequent full Merkle rebuild proved a second, real V2 state-integrity failure: at `71 185 159`, calculated root `0x0edeba…` differs from canonical header/public-RPC root `0x68efa1…`; pipeline therefore unwinds to genesis. Direct layout mutation is now blocked and `db migrate-v2` can resume partial ChangeSet copies as preventive hardening, but neither change explains this incident. Root-cause isolation for the V2 state/static-file data route and a clean restart-to-external-backfill proof remain open. |
