...
| --- | --- | --- |
| Storage v2 | ChangeSets/Senders stubs could corrupt Headers; manual layout flips could create MDBX/static-file/RocksDB hybrids | `fix(storage): AccountChangeSets stub…` → `feat(storage): … STOR-004/005/006`; direct `storage_v2` changes now refuse and `db migrate-v2` resumes partial ChangeSet copies |
...
| Copilot Session 13 activity (Storage-v2 recovery, root-cause analysis and fixes, 2026-09-02) | Journal/Mimir diagnosis of the receipt-static-file unwind; Storage v2 migration/consistency changes; source-level root-cause analysis on `main` yielding two fixes (static-file block-index underflow `34f19ebdd4`, slot-preimage DB port `333ba71e10`); six reactivated preimage regression tests plus `test_pipeline`/`test_pipeline_v2`, `cargo check --workspace` and two `make maxperf-op` builds. Per-session billed-token telemetry is unavailable; no incremental monetary estimate is claimed. |
...
| Storage-v2 recovery / Session 13 (2026-09-02, Root-Cause 16:30 CEST) | Der Archive-Datadir läuft **durchgehend mit `storage_v2=true` seit mindestens 2026-08-14**; kein manueller Layoutwechsel. Die Ursachenanalyse auf `main` fand zwei Portierungsdefekte: (1) `StaticFileProvider::update_index` verschlüsselte den Block-Index unter `segment_max_block` statt unter dem Range-Ende, wodurch `find_fixed_range_with_block_index` einen u64-Underflow auslöst — im Release-Build ein **stiller Wrap**, der vorhandene Static-File-Daten als fehlend meldet (`segment=Receipts` @ `71 185 160`, Auslöser des Unwinds `174 027 661 → 71 185 159`); behoben in `34f19ebdd4`. (2) Die Slot-Preimage-DB aus Upstream #22379 war nie portiert, nur ihre Tests waren per `#[ignore]` stillgelegt — dadurch blieben V2-Wipe-ChangeSets unvollständig; nachgeholt in `333ba71e10`, 6/6 Tests grün. Da opBNB Cancun bereits 2024-06-20 aktiviert, ist (2) **nicht** ursächlich für den Merkle-Mismatch bei `71 185 159` (post-Cancun), aber Pflicht für den Re-Sync ab Genesis. Neuer aktiver Blocker **PORT-STAGE-006**: nach abgebrochenem Unwind springt `StorageHashing` unter V2 per Forward-No-op von `70 885 156` auf `71 185 159`, ohne den Hashed State zu rekonstruieren — der laufende Repair-Pass wird daher voraussichtlich erneut fehlschlagen. |
