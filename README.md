...
| Compile / runnable milestone (2026-08-10, Session 9) | StorageChangeSets SF (**PORT-STOR-006**); stages nextest **106/106** (8 skipped deferred); op-primitives/consensus/evm/node/rpc nextest green; EF fixtures **v17.0** + Bytecode Compact re-analyze → **61/62** suites (`valid_blocks` was nextest-timeout). Catch-up/full sync = **human-owned** (see `plan.md`). |
