...
| Copilot Session 13 activity (Storage-v2 recovery and backfill analysis, 2026-09-02) | Journal/Mimir diagnosis of the receipt-static-file unwind; Storage v2 migration/consistency changes; focused CLI tests and one `make maxperf-op` build; restart/Engine Header-backfill trace. Per-session billed-token telemetry is unavailable; no incremental monetary estimate is claimed. |
...
| Storage-v2 recovery / Session 13 (2026-09-02, 14:23 CEST) | The invalid manual `storage_v2` metadata flip exposed a missing-receipts static-file invariant and forced dependent stages from **174 027 661** to **71 185 159**. Direct layout mutation is blocked; `db migrate-v2` resumes partial Account-/StorageChangeSet static files; prune checkpoints count as intentional static-file coverage. On restart, consistency repair deliberately starts a local backfill targeting header **71 185 159** (`0x005603…`), so Headers correctly reports that target as reached. The CL then supplies heads around **180.94M**; their FCUs receive `SYNCING` while the exclusive repair pipeline runs and are retained as the latest Engine sync target. After the Merkle rebuild of **2.84B** hashed entities makes the pipeline idle, Headers must begin the queued external backfill. That transition, full migration crash-resume semantics, and end-to-end restart validation remain open. `make maxperf-op`, focused CLI tests, and provider/CLI checks passed. |
