...
| --- | --- | --- |
| Storage v2 | ChangeSets/Senders stubs could corrupt Headers; manual layout flips could create MDBX/static-file/RocksDB hybrids | `536fa5d09c fix(storage): stop AccountChangeSets SF stub from corrupting Headers` → `34d4ac3d5e feat(storage): port StorageChangeSets static-file segment for v2`; direct `storage_v2` changes now refuse and `db migrate-v2` resumes partial ChangeSet copies |
| Consensus | Eth second-granularity timestamp rejected valid opBNB equal-second blocks | `33695c3999 fix(target-chain): milli-timestamp consensus + engine backfill for live sync` |
| Engine | Tip chase / no pipeline backfill after FCU | same commit `(ENGINE-001)` + tip seed in P2P-003 |
| eth/69 (+) | Tip hash without number; peers without range; Empty→Ban; BlockRange unused | `c22d84498f fix(net): PORT-P2P-003 reachable headers + tip seed` |
| Headers downloader | Cap re-loop / Falling stuck at `total=1` | `d8a33ce8e fix(net): prime Falling after Cap Number tip` |
| NAT | `--nat any` without real UPnP; undialable announce | `52feeeb23 feat(net): UPnP NAT map + eth/69 handshake breach; docs P2P-002 live` |
| Execution | Receipt-root fail @ Hertz/`0x67` (`21591154`) | `26e28074ab fix(op)+docs: CometBFT Hertz overlay for PIPE-014 receipt root` |
| Wright L1 fee | Skip semantics must match op-geth (`gasPrice==0`) | `cda93879cb fix(op-revm): add skip l1 data fee support` |
...
| Copilot Session 13 activity (Storage-v2 recovery, root-cause analysis and fixes, 2026-09-02) | Journal/Mimir diagnosis of the receipt-static-file unwind; Storage v2 migration/consistency changes; source-level root-cause analysis on `main` yielding two fixes (static-file block-index underflow `9c44ef8104`, slot-preimage DB port `ee3f376681`); six reactivated preimage regression tests plus `test_pipeline`/`test_pipeline_v2`, `cargo check --workspace` and two `make maxperf-op` builds. Per-session billed-token telemetry is unavailable; no incremental monetary estimate is claimed. |
...
| Live sync (2026-09-01 ~18:52 CEST) | Headers/Bodies/SenderRecovery **174 027 661**; Exec **`65 828 907`** (~38 % Headers tip); **past Wright**; rate cooled ~**19–33 blk/s** (24 h ~22); ETA Headers tip **~1¼–2¼ Mo** (current bands). Peers 16; validation **0**. Snapshot: `files/opbnb-archive-sync-snapshot-20260820.json`. |
| Storage-v2 recovery / Session 13 (2026-09-02, Root-Cause 16:30 CEST) | Der Archive-Datadir läuft **durchgehend mit `storage_v2=true` seit mindestens 2026-08-14**; kein manueller Layoutwechsel. Die Ursachenanalyse auf `main` fand zwei Portierungsdefekte: (1) `StaticFileProvider::update_index` verschlüsselte den Block-Index unter `segment_max_block` statt unter dem Range-Ende, wodurch `find_fixed_range_with_block_index` einen u64-Underflow auslöst — im Release-Build ein **stiller Wrap**, der vorhandene Static-File-Daten als fehlend meldet (`segment=Receipts` @ `71 185 160`, Auslöser des Unwinds `174 027 661 → 71 185 159`); behoben in `9c44ef8104`. (2) Die Slot-Preimage-DB aus Upstream #22379 war nie portiert, nur ihre Tests waren per `#[ignore]` stillgelegt — dadurch blieben V2-Wipe-ChangeSets unvollständig; nachgeholt in `ee3f376681`, 6/6 Tests grün. Die spätere Datadir-Obduktion belegte einen Misch-State (`HashedAccounts` real `71 242 925`, `HashedStorages` real `70 885 156`, Static Files `71 185 159`); Reparatur war wegen gekürzter AccountChangeSets nicht mehr lokal möglich, daher Re-Sync ab Genesis. Zusätzliche lokale Guards: Storage-V2-aware `stage drop Execution`, lauter `remove_state_above`-Abort bei Execution>Blockdaten, Startup-Abbruch bei Execution==Header-Tip aber Hashing darunter, und Hashed-State-Clear bei Hashing-Unwind auf Genesis (`4e4c078555`). |
| Commits | See `git log --first-parent main`; public history was rebuilt as sanitized local-only commits on 2026-09-02. |
