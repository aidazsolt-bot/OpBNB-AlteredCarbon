...
| opBNB Reth benchmark comparison + Execution-stage bottleneck root cause (2026-09-05) | Compared our Execution-stage rate (~37-38 blocks/s) against the official BNB Chain Reth v1.0 benchmark (opBNB, AWS i4g.4xlarge: 16 vCPU/128GiB, single-tenant): stage-sync-from-genesis totalled 53.38h (archive) at ~690 MGas/s historical backfill, but the more comparable **live-sync** numbers show execution alone at ~134-139 MGas/s dropping to only **~43-44 MGas/s once merklization + MDBX commit are included** — the official post confirms *"mdbx is not a write-friendly database... a challenge that becomes more pronounced for fast-blocking layer 2 solutions like opBNB"*. That benchmark's live-sync window was blocks 30.43M-30.47M — chain tip ~30.5M at the time vs. our 181M-block target now (~5.9x longer chain) — so its 43-44 MGas/s figure is likely an optimistic upper bound for us, not pessimistic. Conclusion: the per-block MDBX commit latency is an architectural bottleneck in reth's Execution stage, not a function of available CPU/RAM/NVMe headroom — so our host being shared with ~15 other concurrently running chain nodes is not the primary driver of the slower rate, contrary to an earlier host-contention hypothesis. Anonymized host class: single 16-core/32-thread x86 server, several hundred GiB RAM, multiple consumer NVMe drives, multi-tenant. |
