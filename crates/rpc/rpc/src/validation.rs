    ...
    T: PayloadTypes<ExecutionData = ExecutionData>,
{
    ...
    /// Validates a block submitted to the relay (v6).
    async fn validate_builder_submission_v6(
        &self,
        _request: BuilderBlockValidationRequestV6,
    ) -> RpcResult<()> {
        Err(rpc_error_with_code(-32601, "validation_validateBuilderSubmissionV6 is not available"))
    }
}
