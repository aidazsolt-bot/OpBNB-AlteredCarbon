...
use reth_primitives_traits::{InMemorySize, NodePrimitives, TxTy};
use reth_rpc::{
    ...
    RpcModuleBuilder<N, Provider, Pool, Network, EvmConfig, Consensus>
{
    ...
    where
        N: NodePrimitives,
        EvmConfig: ConfigureEvm<Primitives = N> + Clone,
        Provider: Clone,
        Pool: Clone,
        Network: Clone,
        RpcNodeCoreAdapter<Provider, Pool, Network, EvmConfig>:
...
pub struct RpcServerConfig<RpcMiddleware = Identity> {
    ...
    /// Whether built-in RPC request metrics are enabled.
...
impl<RpcMiddleware> RpcServerConfig<RpcMiddleware> {
    ...
    /// Configure whether built-in RPC request metrics are enabled.
    pub const fn with_rpc_metrics_enabled(mut self, rpc_metrics_enabled: bool) -> Self {
        self.rpc_metrics_enabled = rpc_metrics_enabled;
        self
    }
    ...
    fn optional_request_metrics(
        rpc_metrics_enabled: bool,
        module: Option<&RpcModule<()>>,
        build: fn(&RpcModule<()>) -> RpcRequestMetrics,
    ) -> RpcRequestMetrics {
        if rpc_metrics_enabled {
            module.map(build).unwrap_or_default()
        } else {
