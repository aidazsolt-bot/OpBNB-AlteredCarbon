    ...
    + 'static
{
    ...
    /// Ommer header type.
    type OmmerHeader: FullBlockHeader;
    ...
    /// - Ommers
    ///
    /// Note: This conversion can be incomplete. It is not expected that this `Body` is the same
    /// as [`alloy_consensus::BlockBody`] only that it can be converted into it which is useful
    /// for the `eth_` RPC namespace (e.g. RPC block).
    ...
    fn contains_transaction_type(&self, tx_type: u8) -> bool {
        self.transactions_iter().any(|tx| tx.transaction().is_type(tx_type))
    }
    ...
    fn blob_gas_used(&self) -> u64 {
        self.transactions_iter().filter_map(|tx| tx.transaction().blob_gas_used()).sum()
    }

    /// Returns whether or not the block body contains any blob transactions.
    fn has_blob_transactions(&self) -> bool {
        self.transactions_iter().any(|tx| tx.transaction().blob_gas_used().is_some())
    }
    ...
    fn blob_versioned_hashes_iter(&self) -> impl Iterator<Item = &B256> + '_ {
        self.transactions_iter().filter_map(|tx| tx.transaction().blob_versioned_hashes()).flatten()
    }
...
/// This is a helper alias to make it easy to refer to the inner `Transaction` associated type of
/// a given type that implements [`BlockBody`].
pub type BodyTx<N> = <N as BlockBody>::Transaction;

/// This is a helper alias to make it easy to refer to the inner `OmmerHeader` associated type of
/// a given type that implements [`BlockBody`].
