...
use alloc::vec::Vec;
use alloy_consensus::{transaction::TransactionMeta, BlockHeader as _};
...
/// ## Recovery
///
/// Sender recovery is fallible and can fail if any of the transactions fail to recover the
...
impl<B: Block> RecoveredBlock<B> {
    ...
    /// not, using [`SignedTransaction::recover_signer`] to recover the senders.
    pub fn try_new(
        block: B,
        senders: Vec<Address>,
        hash: BlockHash,
    ...
    /// A safer variant of [`Self::new`] that checks if the number of senders is equal to
    /// the number of transactions in the block and recovers the senders from the transactions, if
    /// not, using [`SignedTransaction::recover_signer_unchecked`] to recover the senders.
    ...
    pub fn clone_sealed_header(&self) -> SealedHeader<B::Header> {
        self.block.clone_sealed_header()
    }
    ...
    /// Consumes the type and returns an iterator over all [`Recovered`] transactions in the
    /// block.
    #[inline]
    ...
    pub fn into_transactions(self) -> Vec<<B::Body as BlockBody>::Transaction> {
        self.block.into_block().into_body().into_transactions()
    }
}
...
#[cfg(any(test, feature = "test-utils"))]
impl<B: crate::block::TestBlock> RecoveredBlock<B>
where
    B::Header: crate::header::test_utils::TestHeader,
{
...
impl<'a, B: Block> IndexedTx<'a, B> {
    ...
    pub fn meta(&self) -> TransactionMeta {
        TransactionMeta {
            ...
            base_fee: self.block.header().base_fee_per_gas(),
