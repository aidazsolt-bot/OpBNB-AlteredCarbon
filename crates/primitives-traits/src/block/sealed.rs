...
use crate::{
    ...
    Block, BlockBody, BlockHeader, GotExpected, InMemorySize, SealedHeader,
};
use alloc::vec::Vec;
...
impl<B: Block> SealedBlock<B> {
    ...
    pub fn set_header(mut self, header: B::Header) -> Self {
        self.header = SealedHeader::seal_slow(header);
        self
    }
    ...
    pub fn clone_block(&self) -> B {
        B::new(self.header.header().clone(), self.body.clone())
    }
...
impl<B: crate::block::TestBlock> SealedBlock<B> {
