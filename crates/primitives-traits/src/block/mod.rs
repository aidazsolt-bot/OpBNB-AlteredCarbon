...
use crate::{
    ...
    InMemorySize, SealedHeader, SignedTransaction,
};

/// Helper trait, unifies behaviour required of a block header.
///
/// Kept as a thin alias over [`crate::BlockHeader`] for compatibility with call sites written
/// against the previous, smaller `Block` trait shape.
pub trait Header: BlockHeader {}

impl<T> Header for T where T: BlockHeader {}
    ...
    + Decodable
{
    ...
    /// Create a new sealed block instance from a sealed header and the block body.
    ...
    /// Creates the [`SealedBlock`] from the block without calculating the hash upfront.
    ...
    /// - Ommers
    ///
    /// Note: This conversion can be incomplete. It is not expected that this `Block` is the same
    /// as [`alloy_consensus::Block`] only that it can be converted into it which is useful for
