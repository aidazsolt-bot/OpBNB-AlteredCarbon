...
impl<T: InMemorySize + ?Sized> InMemorySize for &T {
    fn size(&self) -> usize {
        (**self).size()
    }
}

impl<T: InMemorySize + ?Sized> InMemorySize for alloc::sync::Arc<T> {
    fn size(&self) -> usize {
        (**self).size()
    }
}

impl<T: InMemorySize + ?Sized> InMemorySize for alloc::boxed::Box<T> {
    fn size(&self) -> usize {
        (**self).size()
    }
}
