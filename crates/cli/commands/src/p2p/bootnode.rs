...
mod tests {
    ...
    fn repeated_nat_uses_matching_addr_family_as_primary() {
        ...
        assert_eq!(nat.resolver, NatResolver::ExternalIp("<ipv6><ipv6>".parse().unwrap()));
        assert_eq!(nat.advertised_ips.len(), 2);
    }

    #[test]
    fn repeated_nat_requires_extip_values() {
        let command = Command::parse_from(["reth", "--nat", "any", "--nat", "extip:<ipv6><ipv6>"]);
    ...
    fn repeated_nat_requires_distinct_ip_families() {
        let command =
            Command::parse_from(["reth", "--nat", "extip:<ipv4>", "--nat", "extip:<ipv4>"]);
    ...
    fn single_extip_is_advertised() {
        let command =
            Command::parse_from(["reth", "--addr", "0.0.0.0:30301", "--nat", "extip:<ipv4>"]);

        let nat = command.resolved_nat().unwrap();

        assert_eq!(nat.advertised_ips, vec!["<ipv4>".parse::<IpAddr>().unwrap()]);
    }
    ...
    fn discv5_advertises_single_extip_of_other_family_than_addr() {
        ...
        assert_eq!(enr.ip6(), Some("<ipv6><ipv6>".parse().unwrap()));
    ...
    fn discv5_config_single_stack_ipv4_uses_addr_port() {
        ...
        assert_eq!(enr.ip4(), Some("<ipv4>".parse().unwrap()));
    ...
    fn single_opposite_family_extip_does_not_drive_discv4() {
        ...
        assert_eq!(nat.advertised_ips, vec!["<ipv6><ipv6>".parse::<IpAddr>().unwrap()]);
    }
    ...
    fn discv4_config_advertises_only_the_addr_family() {
        ...
        assert_eq!(nat.resolver, NatResolver::ExternalIp("<ipv4>".parse().unwrap()));
