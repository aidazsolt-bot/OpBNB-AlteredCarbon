...
pub struct OpBlockExecutionCtx {
    ...
    /// Whether this is opBNB's exact PreContractForkBlock activation block.
    ///
    /// The chain-specific condition is computed by the Reth integration; this executor applies
    /// the resulting irregular state transition before any system or user transaction.
    pub apply_pre_contract_hardfork: bool,
    ...
    Spec: OpHardforks,
{
    ...
    fn apply_pre_execution_changes(&mut self) -> Result<(), BlockExecutionError> {
        ...
        if self.ctx.apply_pre_contract_hardfork {
            precontract::apply_pre_contract_hardfork(self.evm.db_mut())
                .map_err(BlockExecutionError::other)?;
        }
