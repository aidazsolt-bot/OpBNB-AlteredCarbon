...
use super::{dedup::DuplicateTracker, error::OpBnbPrecompileError};
...
/// BSC pre-Hertz variant: forces `validatorSetChanged = false` (buggy post-update check).
/// **Not** used on opBNB — op-geth always returns the pre-update flag (Hertz semantics).
#[allow(dead_code)] // kept for BSC parity / unit tests; overlay injects Hertz instead
...
fn convert_light_block_from_proto(light_block_proto: &TmLightBlock) -> ConvertLightBlockResult {
    let signed_header =
        match SignedHeader::try_from(light_block_proto.signed_header.as_ref().unwrap().clone()) {
            Ok(sh) => sh.clone(),
            Err(_) => return Err(OpBnbPrecompileError::InvalidInput.into()),
        };
...
fn decode_light_block_validation_input(
    ...
    if input_length < CONSENSUS_STATE_LENGTH_BYTES_LENGTH {
        return Err(OpBnbPrecompileError::InvalidInput.into());
    }
...
impl ConsensusState {
    ...
    fn apply_light_block(&mut self, light_block: &LightBlock) -> Result<bool, PrecompileHalt> {
        ...
        if self.height + 1 == light_block.height().value() {
            if self.next_validator_set_hash.ne(light_block
                ...
                .as_bytes())
            {
                return Err(OpBnbPrecompileError::CometBftApplyBlockFailed.into());
            }
    ...
    fn encode(&self) -> Result<Bytes, PrecompileHalt> {
        ...
        if serialize_length > MAX_CONSENSUS_STATE_LENGTH as usize {
            return Err(OpBnbPrecompileError::CometBftEncodeConsensusStateFailed.into());
        }
...
fn decode_consensus_state(
    ...
    for i in 0..validator_set_length {
        ...
        let pk = match PublicKey::from_raw_ed25519(&validator[..VALIDATOR_PUBKEY_LENGTH as usize]) {
            Some(pk) => pk,
            None => return Err(OpBnbPrecompileError::InvalidInput.into()),
        };
...
mod tests {
    ...
    fn test_cometbft_light_block_validate() {
        {
            ...
            let result = cometbft_light_block_validation_run(&input, 100_000, 0);
            let output = result.expect("cometbft_light_block_validation_run failed");
            assert_eq!(output.gas_used, 3_000);
            assert_eq!(output.bytes, except_output);
    ...
    fn test_cometbft_light_block_validate_before_hertz() {
        ...
        let result = cometbft_light_block_validation_run_before_hertz(&input, 100_000, 0);
        let output = result.expect("cometbft_light_block_validation_run failed");
        assert_eq!(output.gas_used, 3_000);
    ...
    /// op-geth / opBNB use Hertz semantics: when the validator set actually changes, byte0 of
    /// the precompile output is `1`. BEFORE_HERTZ forces `0` (BSC pre-Hertz bug) — that caused
    /// under-gas `syncLightBlock` receipts (PIPE-014 @ 21591154).
    #[test]
    fn hertz_reports_validator_set_changed_when_before_hertz_hides_it() {
        let cs = hex!("");
        let light_block = hex!("");
        let mut input = vec![0u8; 32];
        input[24..32].copy_from_slice(&(cs.len() as u64).to_be_bytes());
        input.extend_from_slice(&cs);
        input.extend_from_slice(&light_block);

        let before = cometbft_light_block_validation_run_before_hertz(&input, 100_000, 0)
            .expect("before_hertz");
        let hertz = cometbft_light_block_validation_run(&input, 100_000, 0).expect("hertz");
        assert!(before.is_success() && hertz.is_success());
        assert_eq!(before.bytes[0], 0, "BEFORE_HERTZ hides validator-set change");
        assert_eq!(hertz.bytes[0], 1, "Hertz / op-geth reports validator-set change");
        // Consensus-state payload after the 32-byte header should still match.
        assert_eq!(&before.bytes[32..], &hertz.bytes[32..]);
    }
