...
use crate::{precompiles::BscPrecompiles, transaction::BscTxEnv};
...
pub type BscContext<DB> = Context<BlockEnv, BscTxEnv, CfgEnv<SpecId>, DB>;
...
impl<DB: Database, I> BscEvm<DB, I> {
    ...
    pub fn new(env: EvmEnv<SpecId>, db: DB, inspector: I, inspect: bool, trace: bool) -> Self {
        let precompiles =
...
impl<DB: Database, I> BscEvm<DB, I> {
    ...
    fn detect_system_transaction(&self, tx: &BscTxEnv) -> bool {
        use reth_bsc_primitives::system_contracts::is_invoke_system_contract;
