...
impl BscTxEnv {
    pub fn new(base: RevmTxEnv) -> Self {
        Self { base, is_system_transaction: false }
    }
}

impl Transaction for BscTxEnv {
    type AccessListItem<'a> = <RevmTxEnv as Transaction>::AccessListItem<'a>;
    type Authorization<'a> = <RevmTxEnv as Transaction>::Authorization<'a>;
...
impl FromRecoveredTx<TransactionSigned> for BscTxEnv {
    fn from_recovered_tx(tx: &TransactionSigned, sender: Address) -> Self {
        Self::new(RevmTxEnv::from_recovered_tx(tx, sender))
    }
}
...
impl FromTxWithEncoded<TransactionSigned> for BscTxEnv {
    fn from_encoded_tx(tx: &TransactionSigned, sender: Address, _encoded: Bytes) -> Self {
        let base = match tx.clone().into_typed_transaction() {
            reth_primitives::Transaction::Legacy(tx) => RevmTxEnv::from_recovered_tx(&tx, sender),
            reth_primitives::Transaction::Eip2930(tx) => RevmTxEnv::from_recovered_tx(&tx, sender),
            reth_primitives::Transaction::Eip1559(tx) => RevmTxEnv::from_recovered_tx(&tx, sender),
            reth_primitives::Transaction::Eip4844(tx) => RevmTxEnv::from_recovered_tx(&tx, sender),
            reth_primitives::Transaction::Eip7702(tx) => RevmTxEnv::from_recovered_tx(&tx, sender),
        };
...
impl<Spec, BlockEnv: alloy_evm::env::BlockEnvironment> alloy_evm::rpc::TryIntoTxEnv<BscTxEnv, Spec, BlockEnv>
    for alloy_rpc_types_eth::TransactionRequest
{
    type Err = <alloy_rpc_types_eth::TransactionRequest as alloy_evm::rpc::TryIntoTxEnv<
        RevmTxEnv,
        Spec,
        BlockEnv,
    >>::Err;
    ...
    fn try_into_tx_env(
        ...
        Ok(BscTxEnv {
            base: <alloy_rpc_types_eth::TransactionRequest as alloy_evm::rpc::TryIntoTxEnv<
                RevmTxEnv,
                Spec,
                BlockEnv,
            >>::try_into_tx_env(self, evm_env)?,
