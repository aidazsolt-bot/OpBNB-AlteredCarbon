...
pub struct NetworkArgs {
    ...
    ///
    /// --trusted-peers enode://abcd@<ipv4>:30303
...
mod tests {
    ...
    fn parse_trusted_peer_args() {
        let args =
            ...
            "reth",
            "--trusted-peers",
            "enode://d860a01f9722d78051619d1e2351aba3f43f943f6f00718d1b9baa4101932a1f5011f16bb2b1bb35db20d6fe28fa0bf09636d26a87d31de9ec6203eeedb1f666@<ipv4>:30303,enode://22a8232c3abc76a16ae9d6c3b164f98775fe226f0917b0ca871128a74a8e9630b458460865bab457221f1d448dd9791d24c4e5d88786180ac185df813a68d4de@<ipv4>:30303"
        ])
        .args;
        ...
        assert_eq!(
            args.trusted_peers,
            vec![
            "enode://d860a01f9722d78051619d1e2351aba3f43f943f6f00718d1b9baa4101932a1f5011f16bb2b1bb35db20d6fe28fa0bf09636d26a87d31de9ec6203eeedb1f666@<ipv4>:30303".parse().unwrap(),
            "enode://22a8232c3abc76a16ae9d6c3b164f98775fe226f0917b0ca871128a74a8e9630b458460865bab457221f1d448dd9791d24c4e5d88786180ac185df813a68d4de@<ipv4>:30303".parse().unwrap()
            ]
        );
    }
    ...
    fn parse_netrestrict_single_network() {
        ...
        assert!(ip_filter.is_allowed(&"<ipv4>".parse().unwrap()));
        assert!(!ip_filter.is_allowed(&"<ipv4>".parse().unwrap()));
    }
    ...
    fn parse_netrestrict_ipv6() {
        let args =
            CommandParser::<NetworkArgs>::parse_from(["reth", "--netrestrict", "<ipv6>/32"])
                .args;

        let ip_filter = args.ip_filter().unwrap();
        assert!(ip_filter.has_restrictions());
        assert!(ip_filter.is_allowed(&"<ipv6><ipv6>".parse().unwrap()));
        assert!(!ip_filter.is_allowed(&"<ipv6><ipv6>".parse().unwrap()));
    }
    ...
    fn network_config_preserves_basic_nodes_from_peers_file() {
        let enode = "enode://6f8a80d14311c39f35f516fa664deaaaa13e85b2f7493f37f6144d86991ec012937307647bd3b9a82abe2974e1407241d54947bbb39763a4cac9f77166ad92a0@<ipv4>:30303?discport=30301";
