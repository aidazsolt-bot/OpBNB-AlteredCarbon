...
| PORT-STOR-005 | AccountChangeSets SF incomplete (Headers corruption) | Missing `.csoff` sidecar / header len / writer heal; stubs wrote Headers | ✅ fixed: SegmentHeader `changeset_offsets_len` + sidecar writer/heal/truncate; `account_changesets_in_static_files() → storage_v2`; StorageChangeSets bleibt MDBX |
...
- Offen danach: ~~echte AccountChangeSets/TransactionSenders-SF-Segmente~~ → PORT-STOR-004/005; StorageChangeSets-SF optional; EF/nextest; Archive-Sync; live P2P nach Rebuild.
