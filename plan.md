...
| **Snap (nach Sync-Gates)** | **PIPE-015…017** | **FLOW-SNAP-01…06** | **100 % Spec snap/1+2 Client+Peer** — siehe Roadmap FEAT-SNAP-001; **nicht** Ersatz für Archive-History |
...
| PORT-FLOW-S04 | V1→V2 layout migration / repair backfill | `storage_v2` darf erst nach vollständiger Datenmigration gesetzt werden; ein Teilabbruch muss aus MDBX in die bestehenden Zielsegmente fortsetzbar sein. Prune-Checkpoints repräsentieren absichtlich entfernte Static-File-Abdeckung, nicht Korruption. Bei einem unterbrochenen Reparaturlauf startet `initial_backfill_target` absichtlich am letzten konsistenten Header; eintreffende FCUs werden als neuestes Sync-Ziel gehalten und dürfen nach Pipeline-Idle nicht verloren gehen. | STOR-007/008, ENGINE-001 | 📋 Code: direkte Metadatenmutation gesperrt; ChangeSet-Resume + Prune-Coverage portiert. Live 09-02: lokaler Repair-Target `71185159` (`0x005603…`) korrekt geschlossen, CL-Head ~`180.94M` als `SYNCING` registriert; externer Backfill nach Pipeline-Idle offen. ✅ 09-03 Dev-Host: sauberer `db migrate-v2` E2E @300. Crash-Resume ungetestet. |
| PORT-FLOW-SNAP-01 | Cap-Negotiate | Hello bietet **snap/1 und snap/2** (nicht Cap-only Stub). Mit op-geth-Peers (opBNB = **snap/1**) muss V1 verhandelt werden; V2 mit Peers die EIP-8189 sprechen. Kein `is_exact_eth_snap_v2`-only-Pfad der V1 ausschließt. | P2P-007, PIPE-015 | 🔬 Roadmap FEAT-SNAP-001 |
| PORT-FLOW-SNAP-02 | Server Ranges | `GetAccountRange` / `GetStorageRanges` / `GetByteCodes` aus Hashed+Trie (+ Bytecodes) mit korrekten Boundary-Proofs und Soft-Byte-Limits; kein `UnsupportedCapability` | P2P-007, PIPE-016 | 🔬 heute Stub in `eth_requests.rs` |
| PORT-FLOW-SNAP-03 | Server TrieNodes (snap/1) | `GetTrieNodes`/`TrieNodes` (0x06/0x07) Spec-konform bedienen — Pflicht für opBNB-Interop | P2P-007, PIPE-016 | 🔬 fehlt (Wire V1 fehlt) |
| PORT-FLOW-SNAP-04 | Server BAL (snap/2) | `GetBlockAccessLists` weiter bedienen; Retention/Durability vs Tip-only In-Memory klar dokumentiert | P2P-007, PIPE-016 | 📋 Wire/BAL-Antwort Pfad vorhanden; Prod-Retention offen |
| PORT-FLOW-SNAP-05 | Client Sync SM | Pivot-Root → Account→Storage→Bytecode-Pagination → Heal (TrieNodes V1 / BAL V2) → State commit; Peer-Scoring; Restartability; corrupt-peer handling | P2P-008, PIPE-017 | 🔬 kein SnapSync-Stage |
| PORT-FLOW-SNAP-06 | Pipeline-Kopplung | Snap-State-Bootstrap **ersetzt nicht** Archive-History (Receipts/ChangeSets/History-Stages). Nach Snap: Catch-up Exec/History bis Tip; oder klarer Produktmodus „State-Fast / kein Archive“ | P2P-008, PIPE-017 | 🔬 DoD: Spec-Client + dokumentierte History-Grenze |
...
| **nach Wright-Gate + Sync-Stabilität** | **FEAT-SNAP-001:** 100 % Spec-konforme **snap/1 + snap/2** **Client + Peer** (Serve+Sync); PIPE-015…017 + FLOW-SNAP-01…06; **nicht** Archive-History-Ersatz | Server MVP ~6–10 Wo / Prod ~3–4 Mo; Client MVP ~3–5 Mo / Prod ~5–8+ Mo; beide ~6–10 Mo (1 Senior) | 🔬 Roadmap · Code erst nach PIPE+FLOW-DoD; opBNB-Interop zuerst snap/1 |
...
| PORT-P2P-007 | **Snap Peer (Serve):** kein Spec-konformes snap/1(+2) Serving gegenüber op-geth | Heute: kein snap/1-Wire; snap/2 optional default off; `GetAccountRange`/`GetStorageRanges`/`GetByteCodes` → Stub/`UnsupportedCapability`; kein `GetTrieNodes`. Live: `reth_network_snap_requests_received_total=0`. opBNB-Peers sprechen **eth/68 + snap/1**. | 🔬 FEAT-SNAP-001 · FLOW-SNAP-01…04 · PIPE-015/016 · MVP: Wire V1 → Serve Ranges+TrieNodes → V2/BAL Retention |
| PORT-P2P-008 | **Snap Client:** kein SnapSync / State-Heal über snap | Keine Snap-Stage; nur staged eth Headers/Bodies/Exec. Client = Pivot→Ranges→Bytecode→Heal→Commit + Peer-Scoring/Restart. **≠** Archive Receipts/ChangeSets/History. | 🔬 FEAT-SNAP-001 · FLOW-SNAP-05/06 · PIPE-017 · nach Server-Interop oder parallel mit 2. Dev |
...
| PORT-PIPE-016 | Snap Peer (Serve) | Account/Storage/Bytecode-Ranges + Proofs; V1 TrieNodes; V2 BAL bedienen; Soft-Limits; keine Stub-Rejects | 🔬 Ranges Stub; TrieNodes fehlt; BAL Pfad partiell | **SNAP-02…04** | 🔬 Server-MVP vor Client; Live: Interop vs op-geth Peer |
| PORT-PIPE-017 | Snap Client Sync | Spec-Client: Pivot-Root → paginate → heal → state commit; danach Catch-up oder expliziter Non-Archive-Modus | 🔬 kein SnapSync-Stage | **SNAP-05…06** | 🔬 größter Aufwand; History-Grenze dokumentieren |
...
| **P2** | — | **FEAT-SNAP-001 / P2P-007+008** snap/1+2 Client+Peer (nach Wright-Gate) | PIPE-015…017 + FLOW-SNAP vor Code | 🔬 Roadmap · nicht Exec-P0 |
...
| --- | --- | --- | --- | --- |
| FEAT-HIST-001 | Historische Adress-Analysen auf **≥ Erigon-Niveau** (WalletHistory/OTS + sparse `getLogs`) | CallTrace From/To-Index (+ optional internal frames); echte `ots_searchTransactionsBefore/After` auf Index (nicht 100-Block-Scan); Log Address/Topic inverted index (FilterMaps / EIP-7745, Upstream [#16999](https://github.com/paradigmxyz/reth/issues/16999) / [#18305](https://github.com/paradigmxyz/reth/pull/18305)); optional Sender→Nonce Accessor + `eth_getBlockReceipts`-Pfad | **nach** PORT-P2P-001 live ✅; Archive-Disk-Budget operatorseitig (Call-Index Ballpark 100–400 GiB-Klasse); Upstream-Bezug [#15394](https://github.com/paradigmxyz/reth/issues/15394), stale [#22626](https://github.com/paradigmxyz/reth/pull/22626), [#13499](https://github.com/paradigmxyz/reth/issues/13499) | 📋 geplant · blocked on P2P live · **Upstream beobachten** |
| FEAT-SNAP-001 | **100 % Spec-konforme snap/1 + snap/2 Client + Peer** | **Peer (Serve):** Cap-Negotiate V1+V2; Ranges+Proofs; V1 `GetTrieNodes`; V2 BAL mit klarer Retention. **Client:** voller SnapSync-Zustandsautomat (Pivot→Account/Storage/Bytecode→Heal→Commit), Peer-Scoring, Restartability. **Nicht:** Ersatz für Archive-History (Receipts/ChangeSets/History-Stages). MVP-Reihenfolge: Wire snap/1 → Serve (op-geth-Interop) → Client; snap/2 parallel/niedrige Prio bis op-geth V2 spricht. | **nach** Wright-Gate + Sync-Stabilität; PIPE-015…017 + FLOW-SNAP-01…06 vor Code; Referenz: `bnb-chain/op-geth` snap + ethereum snap Spec / EIP-8189 | 🔬 Roadmap · Aufwand: Server MVP ~6–10 Wo / Prod ~3–4 Mo; Client MVP ~3–5 Mo / Prod ~5–8+ Mo; beide ~6–10 Mo (1 Senior) |
...
**FEAT-SNAP-001 Acceptance (kurz):** (1) Hello verhandelt snap/1 mit op-geth und snap/2 mit V2-Peers. (2) Serve: Ranges+Proofs + V1 TrieNodes (+ V2 BAL) ohne Stub-Reject; Interop-Smoke vs op-geth. (3) Client: Spec-Sync bis State-Root am Pivot; Restart/Heal ok. (4) Dokumentiert: Snap ≠ Archive-History; Catch-up/Non-Archive-Modus klar.

**Upstream-Tracking für den Log-Index (Stand 2026-09-07):** Reth [#16999](https://github.com/paradigmxyz/reth/issues/16999) ist weiterhin offen. Der aktuelle FilterMaps-Ansatz [#26586](https://github.com/paradigmxyz/reth/pull/26586) ist ein offener Draft (`REVIEW_REQUIRED`) und enthält bislang das eigenständige Mapping-/Hashing-Crate, noch keine gemergte RPC-/Storage-Integration. Der frühere Versuch [#18305](https://github.com/paradigmxyz/reth/pull/18305) wurde geschlossen und nicht gemerged. Die bereits gemergten Upstream-Optimierungen [#26999](https://github.com/paradigmxyz/reth/pull/26999), [#27000](https://github.com/paradigmxyz/reth/pull/27000), [#26946](https://github.com/paradigmxyz/reth/pull/26946), [#26971](https://github.com/paradigmxyz/reth/pull/26971) und [#27002](https://github.com/paradigmxyz/reth/pull/27002) verbessern Blocking-Pool-Schutz, Fenster-Scanning, Bloom-Skips, Speicherreservierung und Pruning-Korrektheit, ersetzen aber noch keinen dedizierten Address-/Topic-Index. **Portierungsentscheidung:** Sobald #26586 oder ein Nachfolger brauchbar reviewed/integriert ist und Reorg-/Pruning-/Backfill-Verhalten belastbar sind, muss der Log-Index in diesem Fork nachgezogen und gegen den opBNB-Archive-Benchmark validiert werden; bis dahin kein spekulativer Port des Drafts.
...
| Cursor Session 25 (Chats `6cc355ee…` + Folgetasks **2026-09-12 ~18:30–19:05 CEST**; Doku **2026-09-13 ~08:40 CEST**) | Interaktiv **~3–4 h** (Workspace-Reentry, Checkmk OAuth-MCP, `handshake_reject`, sync-eta Horizon-Fix, README-Grafik, Aufwands-Doku) | Auto/Composer | billed n/a | (Proxy) | Infra-Scaffold; P2P-Handshake-Smoke vs op-geth-Peer ✅; kein Konsens-Code | Live: Tip-43.5 M Heal→Exec; Commit `92990c1861` (example only) |
...
### Wright-Incident → Re-Execution / History-Heal (Maschinenzeit, kein Genesis-Re-Sync)

| Phase | Zeitraum (CEST/UTC) | Maschinen-/Wall-Zeit (Größenordnung) | Ergebnis |
| --- | --- | ---: | --- |
| Receipt-Mismatch + Unwind | **2026-09-09** | Build **22m** + Unwind **72m** + Incident-Analyse **~7 h** Agent-Wall | Unwind auf **`34366337`**; Fix `29d7bfa2dd` |
...
> laut Betreiberangabe weiterhin ca. EUR 70** (Stand 09-13; kein neuer Rechnungsbeleg für Session 22–25).
...
> das korrigiert **Arbeitsstunden**, nicht die EUR-70-Allokation. **Session 24 (+~6 h) + Session 25 (~3–4 h):**
> kumuliert Cursor ~**57–58 h**; EUR **unverändert**. **Reale Copilot-Kosten laut Betreiberangabe: ~EUR 170 kumuliert**
...
| --- | ---: | ---: | ---: |
| Senior Ops / DevOps | **~48–58 h** | **EUR 80–120/h** | **~EUR 3.800–7.000** |
| Senior Reth-/Blockchain-Developer | **~72–82 h** | **EUR 100–150/h** | **~EUR 7.200–12.300** |
| **Gesamt Arbeitswert, mit Vollzählung der Überschneidung** | **~120–140 Rollenstunden** | — | **~EUR 11.000–19.300** |
...
### Infra-Betrieb-Kosten (Restart-/Rebuild-Proxys direkt, Stromkosten — Stand 2026-09-13 **08:45** CEST)
...
## Nächste Schritte (unmittelbar — Stand 2026-09-12 ~09:05 CEST)

> Spiegel [#session-memory](#session-memory): Tip-**43 519 340**-Lauf · TxHash-Heal ~40 % — **nicht** restarten.
