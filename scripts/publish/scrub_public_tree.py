...
LITERALS: list[tuple[str, str]] = [
    ("/root/.cursor/projects/usr-src-Erigon-Binance-reth-bsc-trail-git/agent-transcripts/", "<agent-transcripts>/"),
    ("/root/.copilot/session-store.db", "<copilot-session-store>"),
    ("/usr/src/Erigon/Binance/reth-bsc-trail.git", "<repo>"),
    ("/usr/src/Erigon/Ethereum-MEV-BOT/", "<external-analysis>/"),
    ("/usr/src/Erigon/", "<src-root>/"),
    ("/var/lib/machines/<archive-ct>/var/log/journal/", "<archive-journal>/"),
    ("/var/lib/machines/", "<machine-root>/"),
    ("BSCRethArchiveNode", "<archive-ct>"),
    ("DataDomainNVMER40", "<datadir-vol>"),
    ("DevRocky10", "<host-user>"),
]
...
def scrub_text(text: str) -> str:
    ...
    text = re.sub(
        r"journalctl\s+-D\s+(?:/var/lib/machines/\S+|<machine-root>\S*)",
        "journalctl -D <archive-journal>",
        text,
    )
    text = re.sub(
        r"`?/var/lib/machines/<archive-ct>/var/log/journal/`?",
        "<archive-journal>",
        text,
    )
    text = re.sub(
        r"http://grafana/api/v1/query",
