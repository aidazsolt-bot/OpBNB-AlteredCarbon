...
-   [Troubleshooting](#troubleshooting)
    -   [Database](#database)
        ...
        -   [Concurrent database access error](#concurrent-database-access-error)
...
### Concurrent database access error

If you encounter an error while accessing the database from multiple processes (shared datadir /
multiple node processes), it is possible the error is related to `PID` namespaces or locking. You might see one of the following error messages.
...
Ensure only one writer opens the database, or use separate datadirs. For more information, check out the `Containers` section in the [libmdbx README](https://github.com/erthink/libmdbx#containers).
