...
:::note
Docker image publish, Hive/Kurtosis image builds, and reproducible Docker builds were **removed**
from this fork (no `Dockerfile*` / compose / `docker.yml`). Prefer native binaries and host CI.
:::

### Integration Testing

- Docker-based **Hive** / **Kurtosis** workflows were removed with the Docker decommission.
